...
namespace ArtemisChroniclesOfTheReefGame.Page
{
    ...
    public class APageMultiplayer : APage, IPage
    {
        ...
        public APageMultiplayer(IPrimitive primitive) : base(primitive)
        {
            ...
            _LobbyList.TimeEvent += () =>
            {
                ...
                {
                    Receiver?.Abort();
                    Receiver = new Thread(() => Client.ReceiveResult()) { Name = "Multiplayer-Receiver", IsBackground = true };
                    Receiver.Start();
                    IsReceive = false;
                }
            };
